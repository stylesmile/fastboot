package io.github.stylesmile.plugin.maven.plugin.tools;
...
import io.github.stylesmile.plugin.maven.plugin.Constant;
    ...
    private static String getSolonMainClass(CtClass ctClass, List<String> mains) throws NotFoundException {
        ...
        for (CtMethod method : methods) {
            if (isMainMethod(method)) {
                ...
                System.out.println("ctClass1： " + ctClass);
                //有注解的为主类
                if (ctClass.hasAnnotation(Constant.START_CLASS_ANNOTATION)) {
                    System.out.println("主类为 ： "+ctClass);
                    return ctClass.getName();
                }
                try {
                    System.out.println("ctClass : " + ctClass.getAnnotations().toString());
